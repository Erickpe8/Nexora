...
export const registrarLogGeneracion = (log: LogGeneracion): void => {
  asegurarArchivo()
  const linea = `${JSON.stringify(log)}\n`
  fs.appendFileSync(rutaLog, linea, 'utf8')
  console.log(
    `🤖 IA | guardadas=${log.publicacionesGuardadas} descartadas=${log.publicacionesDescartadas} duracion=${log.duracionMs}ms`
  )
